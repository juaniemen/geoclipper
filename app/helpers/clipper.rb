...
module Clipper
  ...
  def clipper_now(listaTablas, name, conn)
    ...
    (0...listaTablas.length - 1).each do |i|
      ...
      ogc_fidA = %Q["ogc_fid_#{aux}"]
      ogc_fidB = %Q["ogc_fid_#{listaTablas[i+1]}"]
      ...
      if (i == 0)
        ...
        (select a.ogc_fid as #{ogc_fidA}, b.ogc_fid as #{ogc_fidB}, st_intersection(a.wkb_geometry, b.wkb_geometry) as wkb_geometry
        ...
        namesCol.each_row do |x|
          if x[0] != 'ogc_fid' and x[0] != 'wkb_geometry'
            n.push(x[0])
          end
        end
        ...
        query = "drop table if exists #{aux3};
            create table #{aux3} as
            (select #{string1}, b.ogc_fid as #{ogc_fidB}, st_intersection(a.wkb_geometry, b.wkb_geometry) as wkb_geometry
            from #{aux1} as a, #{aux2} as b
            where st_intersects(a.wkb_geometry, b.wkb_geometry));"
    ...
    conn.exec(%Q[ALTER TABLE "#{name}" ADD COLUMN ogc_fid SERIAL PRIMARY KEY;])
  end
  ...
  def load_tables(tableClipped, arrayData, conn)
    arrayData.each do |table1|
      ...
      puts setOfValues
      get_aux = %Q[UPDATE "#{tableClipped}" AS v SET #{setOfValues} FROM #{table} AS s WHERE v.ogc_fid_#{table} = s.ogc_fid;]
      conn.exec(get_aux)
    end
  end

  def validate_table(name1, conn)
    query_srid_unified = %Q[SELECT UpdateGeometrySRID('#{name1}','wkb_geometry',25830);]
